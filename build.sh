...
#electron-packager . --overwrite --version=1.0.1 --platform=win32 --arch=x64 --out=$builddir
#(cd $builddir && zip -r $pubdir/kdinstall-win32-x64.zip kdinstall-win32-x64)

#electron-packager . --overwrite --platform=darwin --arch=x64 --out=$builddir
#(cd $builddir && zip -r $pubdir/kdinstall-darwin-x64.zip kdinstall-darwin-x64)
